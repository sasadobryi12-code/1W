...
Я открыл бинарь в IDA и сначала посмотрел WinMain, понял, что там только создаётся окно и к проверке флага он не относится. Дальше пошёл в оконную процедуру (WndProc), где увидел, что проверка вызывается по WM_COMMAND с ID = 1, то есть при нажатии кнопки, а не при закрытии окна.
